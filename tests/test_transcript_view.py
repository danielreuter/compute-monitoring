...
from event_log import EventLog, EventView, Role, TRANSCRIPT_READERS, VERIFICATION_READERS, DISCLOSURE_READERS
...
class TranscriptViewTest(unittest.TestCase):
    def test_transcript_includes_correct_events(self) -> None:
        log = EventLog(events=[
            InferenceClaimedEvent(
                event_id="claim-1", timestamp=1.0, writer=Role.PROVER, readers=TRANSCRIPT_READERS,
                request_id="req-1", model_id="model-a",
                input_digest="in", output_digest="out",
                artifact_ref=CorrectnessArtifactRef(artifact_id="a1"),
            ),
            MachineAddedEvent(
                event_id="machine-1", timestamp=0.0, writer=Role.PROVER, readers=TRANSCRIPT_READERS,
                machine_id="m1", machine_kind="gpu",
            ),
            NetworkObservationEvent(
                event_id="net-1", timestamp=2.0, writer=Role.VERIFIER, readers=TRANSCRIPT_READERS,
    ...
    def test_transcript_excludes_verification_and_disclosure(self) -> None:
        ...
        log = EventLog(events=[
            CorrectnessCheckRequestedEvent(
                event_id="check-1", timestamp=1.0, writer=Role.VERIFIER, readers=VERIFICATION_READERS,
                session_id="s1", request_id="req-1",
                artifact_ref=ref, strategy="reexecution",
            ),
            CorrectnessArtifactPublishedEvent(
                event_id="pub-1", timestamp=2.0, writer=Role.PROVER, readers=VERIFICATION_READERS,
                session_id="s1", in_reply_to="check-1",
                artifact_ref=ref, bundle=bundle,
            ),
            CorrectnessEvaluatedEvent(
                event_id="eval-1", timestamp=3.0, writer=Role.VERIFIER, readers=VERIFICATION_READERS,
                session_id="s1", request_id="req-1",
                strategy="reexecution", passed=True, details="match",
            ),
            ComplianceEvaluatedEvent(
                event_id="comp-1", timestamp=4.0, writer=Role.VERIFIER, readers=VERIFICATION_READERS,
                passed=True, details="compliant",
            ),
            DisclosurePublishedEvent(
                event_id="disc-1", timestamp=5.0, writer=Role.VERIFIER, readers=DISCLOSURE_READERS,
