...
class RuntimeEngineTest(unittest.TestCase):
    def test_events_are_appended_and_delivered_fifo(self) -> None:
        ...
        e1 = MachineAddedEvent(
            event_id="e1", timestamp=0.0, writer=Side.PROVER, readers=TRANSCRIPT_READERS,
            machine_id="m1", machine_kind="gpu",
        )
        e2 = MachineAddedEvent(
            event_id="e2", timestamp=0.0, writer=Side.PROVER, readers=TRANSCRIPT_READERS,
    ...
    def test_tick_advances_time_and_drains_to_quiescence(self) -> None:
        ...
        e = MachineAddedEvent(
            event_id="tick-event", timestamp=1.0, writer=Side.PROVER, readers=TRANSCRIPT_READERS,
            machine_id="m1", machine_kind="gpu",
        )
